...
/**
 * handle_get_todos - Handles a GET /todos request: retrieves every
 * todo currently stored and sends them back as a JSON array
 * @client_fd: The connected client's socket file descriptor
 *
 * Return: The HTTP status code sent back to the client
 */
static int handle_get_todos(int client_fd)
{
	...
	json = todo_list_to_json();
	send_json_response(client_fd, 200, status_text_for(200), json);
	free(json);
...
void handle_client(int client_fd, const char *ip)
{
	...
	else if (strcmp(path, "/todos") == 0 && strcmp(method, "GET") == 0)
		status = handle_get_todos(client_fd);
	else
	{
